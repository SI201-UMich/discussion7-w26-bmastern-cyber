...
def calculate_avg_price_by_neighbourhood_group_and_room(listings):
    ...
    for d in listings:
        ...
        key = (neighborhood, room)
        if key not in accumulated_price:
            accumulated_price[key] = price
            sum_[key] = 1
        else:
            accumulated_price[key] += price
            sum_[key] += 1
    ...
    for key in accumulated_price:
        average_price[key] = accumulated_price[key] / sum_[key]
...
def write_summary_csv(out_filename, avg_prices):
    ...
    with open(out_filename, 'w', newline = '') as file:
        ...
        for key, value in avg_prices:
            ...
            room_type = key[1]
            average_price = value
            csv_write.writerow([neighbourhood_group, room_type, average_price])
